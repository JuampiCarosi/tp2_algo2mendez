...
void SeCarganCajas() {
  ...
  hash_t *cajas = NULL;
  lista_t *cajas_con_pokemones_indexados = NULL;

  contenedor_cajas_t contenedor = {.cajas = cajas, .cajas_con_pokemones_indexados = cajas_con_pokemones_indexados};

  cargar_cajas(&(contenedor.cajas), 2, nombres_cajas);
  pa2m_afirmar(hash_cantidad(cajas) == 2, "Se cargan 2 cajas");
  pa2m_afirmar(hash_contiene(cajas, "caja1") && hash_contiene(cajas, "caja2"),
               "Se cargan las cajas con el nombre correcto");
  destruir_contenedor_cajas(contenedor);

  cargar_cajas(&cajas, 0, nombres_cajas);
  pa2m_afirmar(cajas == NULL, "No se cargan cajas si la cantidad de nombres es 0");

  cargar_cajas(&cajas, 2, NULL);
  pa2m_afirmar(cajas == NULL, "No se cargan cajas si no se ingresan nombres");
}
...
void SeBuscanCajasConPokemon() {
  ...
  cargar_cajas(&cajas, 2, nombres_cajas);
  indexar_pokemones(cajas, &cajas_con_pokemones_indexados);

  char pokemon1[MAX_NOMBRE_POKEMON] = "pokemon1";
  char pokemon3[MAX_NOMBRE_POKEMON] = "pokemon3";

  lista_t *lista_cajas = buscar_cajas_con_pokemon(cajas_con_pokemones_indexados, pokemon1);
  ...
  lista_cajas = buscar_cajas_con_pokemon(cajas_con_pokemones_indexados, pokemon3);
  pa2m_afirmar(lista_cajas != NULL, "Se crea la lista de cajas con el pokemon");
  pa2m_afirmar(lista_tamanio(lista_cajas) == 1, "Se agrega una caja con el pokemon");
  lista_destruir(lista_cajas);

  pa2m_afirmar(buscar_cajas_con_pokemon(cajas_con_pokemones_indexados, NULL) == NULL,
...
void SeCombinanCajas() {
  ...
  contenedor_cajas_t contenedor = {.cajas = cajas, .cajas_con_pokemones_indexados = NULL};
  ...
  cajas = combinar_cajas(caja1, caja2, caja3, cajas);

  pa2m_afirmar(hash_cantidad(cajas) == 3, "La cantidad de elementos es 3");
  pa2m_afirmar(hash_contiene(cajas, "caja3"), "Se crea la caja combinada con el nombre corecto");

  caja_t *caja_combinada = hash_obtener(cajas, "caja3");

  pa2m_afirmar(caja_cantidad(caja_combinada) == 7, "La cantidad de pokemones dentro de la caja combinada es 7");

  cajas = combinar_cajas(caja1, caja1, caja4, cajas);

  pa2m_afirmar(hash_cantidad(cajas) == 4 && hash_contiene(cajas, "caja4"), "Se puede mezclar la misma caja");

  caja_combinada = hash_obtener(cajas, "caja4");
  ...
  pa2m_afirmar(combinar_cajas(caja1, caja_inexistente, caja3, cajas) == NULL,
               "No se crea la caja combinada si una de las cajas no existe");

  pa2m_afirmar(combinar_cajas(caja1, caja2, caja1, cajas) == NULL,
               "No se crea la caja combinada si se repite el nombre de la caja combinada");

  pa2m_afirmar(combinar_cajas(caja1, caja2, caja3, cajas) == NULL, "No puede sobreescribir un archivo ya escrito");

  pa2m_afirmar(combinar_cajas(NULL, caja2, caja3, cajas) == NULL,
               "No se crea la caja combinada si el nombre de la primera caja es NULL");

  pa2m_afirmar(combinar_cajas(caja1, NULL, caja3, cajas) == NULL,
               "No se crea la caja combinada si el nombre de la segunda caja es NULL");

  pa2m_afirmar(combinar_cajas(caja1, caja2, NULL, cajas) == NULL,
