	...
	for (size_t i = 0; i < hash->capacidad; i++) {
		...
		while (par_actual) {
			hash_t *hash_insertado =
				hash_insertar(hash_aux, par_actual->clave,
					      par_actual->valor, NULL);
...
*/
par_t *insertar_par_en_posicion(par_t *par, const char *clave, void *elemento,
				void **anterior, size_t *cantidad_elementos,
				bool *hay_error)
{
	if (!par) {
	...
	par->siguiente = insertar_par_en_posicion(par->siguiente, clave,
						  elemento, anterior,
						  cantidad_elementos,
						  hay_error);
	return par;
		      ...
		      void **anterior)
{
	...
	if ((double)(hash->cantidad_elementos + 1) / (double)hash->capacidad >=
	    UMBRAL_FACTOR_CARGA) {
	...
	size_t indice =
		jenkins_one_at_a_time_hash((char *)clave) % hash->capacidad;
	hash->pares[indice] = insertar_par_en_posicion(
		hash->pares[indice], clave, elemento, anterior,
		&(hash->cantidad_elementos), &hay_error);
...
*/
void *quitar_nodo(par_t *par, const char *clave, void **eliminado,
		  bool *se_elimino_un_elemento)
{
	if (!par)
		return NULL;
	...
	par->siguiente = quitar_nodo(par->siguiente, clave, eliminado,
				     se_elimino_un_elemento);
	return par;
}
...
void *hash_quitar(hash_t *hash, const char *clave)
{
	...
	hash->pares[indice] = quitar_nodo(hash->pares[indice], clave,
					  &eliminado, &se_elimino_un_elemento);
	if (se_elimino_un_elemento)
...
*/
bool recorrer_posicion(par_t *par, size_t *contador,
			   ...
			   void *aux)
{
	...
	while (i < hash->capacidad && seguir_recorriendo) {
		if (pares[i]) {
			seguir_recorriendo = recorrer_posicion(
				pares[i], &elementos_recorridos, f, aux);
		}
